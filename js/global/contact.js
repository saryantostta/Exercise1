    ...
    $phpcontactform.submit(function(e) {
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                ...
                success: function(d) {
                    if (d == 'success') {
                        if (noredirect) {
                            ...
                            $jscontactresult.fadeIn('slow').html('<div class="alert alert-success mt-3">' + success_msg + '</div>').delay(3000).fadeOut('slow');
                        } else {
                            window.location.href = redirect;
                        }
                    } else {
                        $jscontactresult.fadeIn('slow').html('<div class="alert alert-danger mt-3">' + error_msg + '</div>').delay(3000).fadeOut('slow');
                        if (window.console) {
                ...
                error: function(d) {
                    $jscontactresult.fadeIn('slow').html('<div class="alert alert-danger mt-3"> Cannot access Server</div>').delay(3000).fadeOut('slow');
                    $jscontactbtn.attr("disabled", false);
                    if (window.console) {
